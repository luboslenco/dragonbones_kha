...
@:final class TimelineState {
	...
	static var _pool:Array<TimelineState> = [];
	
	//{ region dragonBones_internal
	public static function borrowObject():TimelineState return _pool.empty() ? new TimelineState() : _pool.pop();
	
	public static function returnObject(timeline:TimelineState) {
		if(!_pool.has(timeline)) {
	...
	public static function getEaseValue(value:Float, easing:Float):Float {
		...
		if (easing > 1) {
			...
			valueEase = Math.sin(value * ConstValues.HALF_PI) - value;
		} else if (easing < 0) {
			valueEase = 1 - Math.cos(value * ConstValues.HALF_PI) - value;
			easing *= -1;
		}
	...
	public var tweenActive:Bool;
	
	var _updateState:Int;
	var _animationState:AnimationState;
	var _bone:Bone;
	var _timeline:TransformTimeline;
	var _currentFrame:TransformFrame;
	var _currentFramePosition:Float;
	...
	public function fadeIn(bone:Bone, animationState:AnimationState, timeline:TransformTimeline) {
		_bone = bone;
		...
		_durationPivot.setTo(0, 0);
		_currentFrame = null;
		var length = _timeline.frameList.length;
		if(length == 0) {
	...
	public function update(progress:Float) {
		...
		if(_updateState > 0) {
			...
			if(progress == 1) {
				progress = 0.99999999;//TODO slavara: wtf?
			}
			
			progress += _timeline.offset;
			var loopCount = Std.int(progress);
			progress -= loopCount;
			
			var playedTime = _totalTime * progress;
			...
			while (_currentFrame == null || playedTime > (_currentFramePosition + _currentFrameDuration) || playedTime < _currentFramePosition) {
				...
				if(_currentFrame != null) {
					...
					_currentFrame = cast(_timeline.frameList[frameIndex], TransformFrame);
				} else {
					frameIndex = 0;
					_currentFrame = cast(_timeline.frameList[0], TransformFrame);
				}
			...
			if(isArrivedFrame) {
				...
				var nextFrame:TransformFrame = cast(_timeline.frameList[frameIndex], TransformFrame);
				if(
					frameIndex == 0 && 
				...
				if(_tweenEasing != _tweenEasing) {
					...
					_durationPivot.setTo(nextFrame.pivot.x - _currentFrame.pivot.x, nextFrame.pivot.y - _currentFrame.pivot.y);
					
					_tweenTransform =  _durationTransform.x != 0
									|| _durationTransform.y != 0
									|| _durationTransform.skewX != 0
									|| _durationTransform.skewY != 0
									|| _durationTransform.scaleX != 0
				...
				if(_tweenTransform) {
					if(_animationState.blend) {
						...
						pivot.setTo(_originPivot.x + _currentFrame.pivot.x, _originPivot.y + _currentFrame.pivot.y);
					} else {
						...
						//pivot.copyFrom(_currentFrame.pivot);
						pivot.setTo(_currentFrame.pivot.x, _currentFrame.pivot.y);
					}
				}
				if(_tweenColor) {
			...
			if(_tweenTransform) {
				...
				var currentPivot = _currentFrame.pivot;
				if(_animationState.blend) {
					...
					pivot.setTo(_originPivot.x + currentPivot.x + _durationPivot.x * progress, 
								_originPivot.y + currentPivot.y + _durationPivot.y * progress);
				} else {
					...
					pivot.setTo(currentPivot.x + _durationPivot.x * progress, currentPivot.y + _durationPivot.y * progress);
				}
			}
			if(_tweenColor) {
				if(_currentFrame.color != null) {
			...
			if(_animationState.blend) {
				...
				pivot.setTo(_originPivot.x, _originPivot.y);
			} else {
				transform.x = 0;
				transform.y = 0;
				transform.skewX = 0;
